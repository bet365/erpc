...
init_per_suite(Config) ->
    ...
    application:set_env(erpc, server_config, [{transport, ssl},
                                              {ssl_options, SSL_opts},
                                              {listen_port, 9090},
                                              {default_node_acl, allow}]),
    _Res = application:ensure_all_started(erpc),
    ok = erpc:connect(?TEST_NODE_1, [{transport, ssl},
                                     {hosts, [{"localhost", 9091}]}]),
    ok = erpc:connect(?TEST_NODE_2, [{transport, ssl},
                                     {hosts, [{"localhost", 9092}]}]),
    ok = erpc:connect(?SELF_CONN,   [{transport, ssl},
...
configure_test_nodes() ->
    ...
    rpc:call(Node_1, application, set_env, [erpc, server_config, 
                                            [{transport, ssl},
                                             ...
                                             {listen_port, 9091},
                                             {default_node_acl, allow}]]),
    rpc:call(Node_1, application, ensure_all_started, [ssl]),
    ...
    rpc:call(Node_2, application, set_env, [erpc, server_config, 
                                            [{transport, ssl},
                                             ...
                                             {listen_port, 9092},
                                             {default_node_acl, allow}]]),
    rpc:call(Node_2, application, ensure_all_started, [ssl]),
    rpc:call(Node_2, application, ensure_all_started, [erpc]).

ssl_options() ->
    Dir = code:lib_dir(erpc, test),
